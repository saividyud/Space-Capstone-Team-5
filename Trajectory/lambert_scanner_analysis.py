        ...
        for t_burn in t_burns:
            ...
            for nu_burn in nu_burns:
                ...
                if len(below_v_threshold_index[0]) == 0:
                    min_rel_velocity = np.nan
                    time_min_rel_velocity = np.nan
                    delta_v_min_rel_velocity = np.nan
                ...
                writer.writerow([t_burn.to(u.day).value, nu_burn.to(u.deg).value, min_rel_velocity, time_min_rel_velocity.to(u.day).value if isinstance(time_min_rel_velocity, u.Quantity) else time_min_rel_velocity, delta_v_min_rel_velocity])
